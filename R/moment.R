...
moment <- function(k, dist, param, lowerDomain = -Inf, upperDomain = Inf, central = FALSE, absolute = FALSE, rel.tol = 1e-6){
  ...
  if(central & absolute){
    if(k == 1){
      ...
      mom <- if(!is.null(names(param))){
        if(all(names(param) %in% names(formals(dist)))){
          med <- integrate(function(x) abs(x) * eval(parse(text = paste0(dist ,"(", names(formals(paste0(dist)))[1], " = ", "x", ", ", paste0(sapply(X = 1:length(param), FUN = function(i) paste(names(param)[i], " = ", param[i])), collapse = ', '), ")"))), lower = lowerDomain, upper = upperDomain, rel.tol = rel.tol)
