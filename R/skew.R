...
#' # parameter of the pareto distribution is \code {a = 4}, and
...
#'         domain = "binom")
#' skew(dist = "nbinom", param = c(size = 15, prob = 0.8),
#'         domain = "binom")
#' @export
skew <- function(dist, param, domain){
  CM3 <- tryCatch(expr = moments(k = 3, dist = dist, param = param, domain = domain,
                                 central = TRUE), error = function(e) "error")
  if(CM3 == "error") stop("The asymptotic method does not converge, the value of the third moment is very large or the third moment of the distribution does not exist. \n Read about the conditions for the existence of the moments of the distribution of interest")
